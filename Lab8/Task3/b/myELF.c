...
void *get_sym_table_Ptr(char *section_name){
    // get the section header table pointer
    Elf32_Shdr *secPtr = (Elf32_Shdr *)(map_start + header->e_shoff);
    // get the section header string table
    Elf32_Shdr *secNames = secPtr+header->e_shstrndx;
...
Elf32_Shdr *get_sym_table_header(char *section_name){
    // get the section header table pointer
    Elf32_Shdr *secPtr = (Elf32_Shdr *)(map_start + header->e_shoff);
    // get the section header string table
    Elf32_Shdr *secNames = secPtr+header->e_shstrndx;
...
void Relocation_Tables(){
    ...
    char * str_table = map_start + secNames->sh_offset;
    ...
    for (i = 0; i < num_of_sections; i++){
        if(sec_header_ptr[i].sh_type == SHT_REL){
            found=true;
            // get the Relocation table
            Elf32_Rel *rel_table = (Elf32_Rel *)(map_start + sec_header_ptr[i].sh_offset);            
            // calculate the size of the table
            int size = sec_header_ptr[i].sh_size / sizeof(Elf32_Rel);

            //This member holds a section header table index link,
            //whose interpretation depends on the section type.
            int index = sec_header_ptr[i].sh_link;

            Elf32_Sym *dyn_sym_table = (Elf32_Sym*)map_start + sec_header_ptr[index].sh_offset;
                
            Elf32_Shdr *dyn_sym_header = &sec_header_ptr[index];
            printf("here1\n");

            char* dyn_str_table = (char*) (map_start + sec_header_ptr[(dyn_sym_header->sh_link)].sh_offset);

            printf("Relocation section '%s' at offset %x contains %d entries\n",
                    str_table + sec_header_ptr[i].sh_name,
            ...
            int k;
            for(k = 0; k < size; k++){
                int index = ELF32_R_SYM(rel_table[k].r_info);
                printf("%09x\t%09x\t%4d\t%06x\t%s\n",
                rel_table[k].r_offset,
                rel_table[k].r_info,
                ELF32_R_TYPE(rel_table[k].r_info),
                dyn_sym_table[index].st_value,
                dyn_str_table+dyn_sym_table[index].st_name);
            }
        }
    }
    if(!found)
        perror("Can't find table");
}

//eqvivalent to Sym.Name:
//map_start+(sec_header_ptr + sec_header_ptr[index].sh_link)->sh_offset + dyn_sym_table[ELF32_R_SYM(rel_table[k].r_info)].st_name
