...
int main(int argc, char **argv){
    ...
    while (1){
        ...
        else if(strncmp(command,"suspend",7) == 0){
            if(newLine->argCount == 3){
                int pid_to_suspend = atoi(newLine->arguments[1]);
                int time_to_suspend = atoi(newLine->arguments[2]);
                pid_t id = fork();
                if(id==0){ /*succeed to fork-> 0 because im at the child*/
                    int result = kill(pid_to_suspend, SIGTSTP);
                    if (result == -1){
                        perror("Unable to suspend");
                        _exit(1);
                    }
                    updateProcessStatus(my_process_list, pid_to_suspend, SUSPENDED);
                    sleep(time_to_suspend);
                    result = kill(pid_to_suspend, SIGCONT);
                    if (result == -1){
                        perror("Unable to continue");
                         _exit(1);
                    }
                    updateProcessStatus(my_process_list, pid_to_suspend, RUNNING);
                    exit(0);
                } 
            }
            else
                fprintf(stderr, "suspend: arguments dont match");
            freeCmdLines(newLine);
        }

        else if(strncmp(command,"kill",4) == 0){
            if(newLine->argCount == 2){
                int pid_to_kill = atoi(newLine->arguments[1]);
                int result = kill(pid_to_kill,SIGINT);
                if(result == -1){
                    perror("kill:");
                    _exit(1);
                }
                updateProcessStatus(my_process_list,pid_to_kill,TERMINATED);
            }
            else if (newLine->argCount < 2)
                fprintf(stderr, "kill: process ID not specified");
            else
                fprintf(stderr, "Kill: to many arguments");
            freeCmdLines(newLine);
        }
